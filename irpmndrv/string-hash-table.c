...
static BOOLEAN _CompareFunction(PHASH_ITEM ObjectInTable, PVOID Key)
{
   ...
   ret = _wcsicmp((PWCHAR)&HashItem->String, str) == 0;
...
static PSTRING_HASH_ITEM _StringToHashItem(PHASH_TABLE Table, PWCHAR String, PVOID Data)
{
   SIZE_T itemLen = 0;
   SIZE_T strLen = 0;
   ...
   strLen = (wcslen(String) + 1) * sizeof(WCHAR);
   itemLen = sizeof(STRING_HASH_ITEM) + strLen;
   ret = HeapMemoryAlloc(itemPoolType, itemLen);
   if (ret != NULL) {
      RtlZeroMemory(ret, itemLen);
      ret->Data = Data;
      RtlCopyMemory(&ret->String, String, strLen);
   }
...
NTSTATUS StringHashTableInsert(PHASH_TABLE Table, PWCHAR String, PVOID Data)
{
   ...
   if (HashItem != NULL) {
      HashTableInsert(Table, &HashItem->Item, &HashItem->String);
